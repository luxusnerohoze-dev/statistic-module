...
async function metaOrganic() {
  ...
  // POZN: page_impressions Meta postupne ruší (do 2026). Je v try/catch (nepovinné) –
  // ak vráti chybu, karta funguje ďalej bez tejto hodnoty.
  try {
    const r2 = await fetch('https://graph.facebook.com/' + V + '/' + m.facebook_page_id + '/insights?metric=page_impressions,page_post_engagements&period=days_28&access_token=' + enc);
    const j2 = await r2.json();
    (j2.data || []).forEach((d) => {
      const vals = d.values || []; const v = vals.length ? vals[vals.length - 1].value : 0;
      if (d.name === 'page_impressions') out.facebook.impressions = num(v);
  ...
  if (igId) {
    try {
      ...
      const r5 = await fetch('https://graph.facebook.com/' + V + '/' + igId + '/insights?metric=reach&period=days_28&access_token=' + enc);
      const j5 = await r5.json();
      if (j5.data && j5.data[0]) { const vv = j5.data[0].values || []; out.instagram.reach = num(vv.length ? vv[vv.length - 1].value : 0); }
    } catch (e) {}
  } else {
...
async function metaAds() {
  ...
  (j.data || []).forEach((row) => {
    ...
    spend += daySpend;
    (row.action_values || []).forEach((a) => { if (/purchase/.test(a.action_type)) val += num(a.value); });
